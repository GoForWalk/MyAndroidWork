...
//                Environment.getExternalStoragePublicDirectory(); API29 부터 deprecated
                try {
                    // 저장경로 가져오기
                    File path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES);
                    File f = new File(path, "external.txt"); // 경로/파일명
                    FileWriter writer = new FileWriter(f, false); // true 이면 append 모드로 생성. // false 이면 writer

                    PrintWriter out = new PrintWriter(writer);
                    out.println(data);
                    out.close(); // close 는 onPause 추천
        ...
        switch (requestCode){
            case REQUEST_CODE:
                ...
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                    ...
                    // 권한이 없으면 더이상 진행 못하게 하던지, 다른 조치를 취한다.
                    // onDestroy();
                }
                return;
