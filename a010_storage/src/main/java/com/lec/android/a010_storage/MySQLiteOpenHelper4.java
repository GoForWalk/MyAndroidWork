...
public class MySQLiteOpenHelper4 extends SQLiteOpenHelper {
    ...
    // 최초에 앱 설치 이후 최초에 데이터베이스가 없는경우, 데이터 베이스 생성을 위해 호출되는 콜백
    // 주로 DDL 등 테이블을 작성하는 코드 작성
    @Override
    public void onCreate(SQLiteDatabase db) {
       Log.d("myapp", "SQLiteOpenHelper] onCreate() 호출");

       String sql = "CREATE TABLE mystudent (" +
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        String sql = "DROP TABLE IF EXISTS mystudent"; // 기존 테이블 삭제
        db.execSQL(sql);
