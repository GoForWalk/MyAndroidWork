...
//        Sensor s = sm.getDefaultSensor(Sensor.TYPE_ORIENTATION);
    ...
    // 화면이 동작하기 직전에 센서자원 흭득.
    @Override
    protected void onResume() {
        super.onResume();
        // 센서값이 변경 되었을 때 마다, 콜백 받기 위한 리스너 등록. SensorEventListener 객체
        sm.registerListener((SensorEventListener)this,
                accelerometer, // 콜백원하는 센서
    ...
    // 화면이 빠져나가기 전에 센서 자원 반납!
    @Override
    protected void onPause() {
        super.onPause();
        //센서에 등록된 리스너 해제!!!!
        sm.unregisterListener(this); // 반납할 센서 - 자신
    }
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if(mGravity != null && mGeomagnetic != null){
            ...
            if(success){
                ...
                float azimuth = orientation[0]; // Z 축
                float pitch = orientation[1]; // X 축
                float roll = orientation[2]; // Y 축
