        ...
        public ViewHolder(@NonNull View itemView) {  // item 레이아웃의 View 객체가 전달됨.
            ...
            btnDelItem.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    // position 정보가 필요하다. (getAdapterPosition)
                    // adapter 로 부터 데이터 삭제도 진행되어야한다.
                    adapter.removeItem(getAdapterPosition()); // 데이터 삭제
                    // 데이터 변경(수정, 삭제, 추가) 내역이 adapter에 반영되어야 정상적으로 작동한다.
            ...
            itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    int position = getAdapterPosition(); // 이 리스너가 장착된 item 의 리스트 상의 position 값
//                    Toast.makeText(v.getContext(), "position: " + position, Toast.LENGTH_SHORT).show();

                    // 아아템을 클릭하면 해당 세부 정보 액티비티로 넘겨주기
