...
public class Main3Activity extends AppCompatActivity {
    ...
    // 삼성폰 : 갤러리 이미지 롱 클릭후 상세정보..
    private String imgPath ="/storage/emulated/0/DCIM/Camera/canada.png";

    private final int PERMISSION_REQUEST_CODE = 101; // 권한 요청 코드 (int)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // '위험권한' 흭득
        // API23 이상에서만 권한 요청(마시멜로우)
        // 권한을 흭득하지 않은 상태에서만 요청
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && ContextCompat.checkSelfPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE)
             != PackageManager.PERMISSION_GRANTED){ // 승인 받지 않았을 때,
