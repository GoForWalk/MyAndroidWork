...
public class Main2Activity extends AppCompatActivity {
        ...
        btnLocate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(btnLocate.isChecked()){// 토글버튼
                    ...
                    try {
                        // 주어진 위치제공자가 기억하는 가장 최근의 위치정보 흭득, 없으면 null 리턴
    ...
    class MyListener implements LocationListener {
        ...
        @Override
        public void onLocationChanged(Location location) { // 위치가 변경되었을때 호출, 위치값은 location 형태로 전달
        ...
        @Override
        public void onProviderEnabled(String provider) { // 위치제공자 가용하게 될때
            Log.d("myapp", "onProviderEnabled, provider: " + provider);
        }

        @Override
        public void onProviderDisabled(String provider) { // 위치제공자 가용하지 못하게 될때
            Log.d("myapp", "onProviderDisabled, provider: " + provider);

        }

        @Override
        public void onStatusChanged(String provider, int status, Bundle extras) {// API29 부터 deprecated....
        ...
        switch (requestCode){
            case REQUEST_CODE:
                if(grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                    Toast.makeText(this, "위치권한 흭득!" , Toast.LENGTH_LONG).show();
                } else {
                    Toast.makeText(this, "위치권한 실패 ㅠㅠ" , Toast.LENGTH_LONG).show();
