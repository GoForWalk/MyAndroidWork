...
//        abar.show(); // 액션바 보이기
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.menu_main3, menu); // 앞 메뉴(R.menu.menu_main3)를 지금의 액티비티 메뉴(menu)에 달아주기

        // #2 : 액션바의 EditText 로 입력 가능
        View v = menu.findItem(R.id.menu_search).getActionView(); // 뷰 객체를 얻어옴!!

        if(v != null){ // 뷰가 null 이 아닐 때,
            EditText editText = v.findViewById(R.id.editText);

            if(editText != null){ // editText 가 입력 되었을 때,

                editText.setOnEditorActionListener(new TextView.OnEditorActionListener() {
                    @Override
                    public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {// 입력되는 순간 나타나는 listener
