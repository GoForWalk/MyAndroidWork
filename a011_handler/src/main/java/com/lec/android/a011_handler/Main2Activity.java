...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 방법3
        BackThread3 thread3 = new BackThread3(handler3); // 메인쓰레드의 Handler 객체를 외부 클래스에 넘겨줌
        thread3.setDaemon(true);
        thread3.start();

        // 방법4.3
        BackThread4 thread4 = new BackThread4(handler4); // 메인쓰레드의 Handler 객체를 외부 클래스에 넘겨줌
    ...
    class BackThread1 extends Thread{
        ...
        public void run(){// 작업 쓰레드
            for(;;){
                ...
                // 방법 2) 메인에 생성된 Handler 객체의 postXXX() 를 통해 Runnable 객체 전달
                backValue2 += 2;
    ...
    Handler handler1 = new Handler(){
        ...
        public void handleMessage(@NonNull Message msg) {
            if(msg.what == 1) {// Message id 가 1 이면,
                tvBackValue1.setText("BackValue1: " + backValue1); // 메인쓰레드의 UI 변경
            }
        }
    };
...
class BackThread3 extends Thread{
    ...
    public void run() {
        ...
        for(;;) {
            ...
            handler.sendMessage(msg);  // 메인쓰레드의 핸들러에 메세지 보내기!!
...
// 방법4.1 : 메인쓰레드의 Handler 를 직접 사용할 수 없는 분리된 작업 쓰레드
