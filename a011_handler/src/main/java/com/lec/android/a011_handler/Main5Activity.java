...
public class Main5Activity extends AppCompatActivity {
    ...
    class BackgroundTask extends AsyncTask<Integer, Integer, Integer>{
        ...
        protected void onPreExecute() {
            Log.d("myapp", "onPreExecute"); // 시작시 태그 가능!!
            super.onPreExecute();
        }

        // 백그라운드 작업. 반드시 구현!!
        @Override
        protected Integer doInBackground(Integer... integers) {// ... :  가변 매개 변수
            // 마지막 integers = Integer[]
            ...
            //** doInBackGround() 에서 시간이 많이 걸리는 ' 통신' 작업이나 복잡한 연산 작업등은 (비동기로) 수행케 해야한다.

        }

        // 백그라운드 작업 도중 (여러번 호출 가능, 진행상황 업데이트, 중간중간 UI 업데이트 시 사용


        @Override
        protected void onProgressUpdate(Integer... values) { // 클래스의 2번째 매개변수 Integer
            Log.d("myapp", "Progress : " + values[0] + "%"); // publishProgress(i)가 보낸 값 = value
            super.onProgressUpdate(values);

            tvBackValue1.setText("onProgressUpdate: " + values[0]); // 여기가 유용함!!
            // Handler 없이도 메인 UI에 접근 가능!!!!!!
        }

        // doInBackground() 완료되면 호출
        @Override
        protected void onPostExecute(Integer integer) { // doInBackGround 에서 return 한 값을 메개변수로 받는다.
            Log.d("myapp", "Result : " + integer);
            super.onPostExecute(integer);

            tvBackValue1.setText("onPostExecute: " + integer); // Handler 없이도 메인 UI에 접근 가능!!!!!!
        }
